...
bool ModeYaw_Tracker::init(bool ignore_checks)
{
    return true;
}
...
void ModeYaw_Tracker::run()
{
    static float braker = 0.0f;
    static float roll_input = 0.0f;
    // 新增：初始化变量以存储从MAVLink消息接收到的throttle值
    ...
    // float target_yaw_rate = target_roll * (1.0f - (pitch_vel2 / 5000.0f)) * g2.command_model_acro_y.get_rate() / 45.0;  //修改这行代码 替换为mavlink中取出的航向角
    float target_yaw_rate = received_yaw_c; 
    ...
    // 根据throttle_c值控制无人机的前进、悬停和后退
    float pilot_throttle_scaled = 0.5f; // 默认悬停

    if (received_throttle_c == 1) {
        pilot_throttle_scaled = 1.0f; // 全速前进
    } else if (received_throttle_c == -1) {
        pilot_throttle_scaled = 0.0f; // 全速后退
    }

    // 保持高度不变的辅助油门
    const float assisted_throttle = get_throttle_assist(vel.z, pilot_throttle_scaled);
...
float ModeYaw_Tracker::get_throttle_assist(float velz, float pilot_throttle_scaled)
{
